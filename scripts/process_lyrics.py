...
def calculate_duration_stats(segments: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for seg in segments:
        ...
        if lyric_count > 0:
            if lang_type == 'western':
                total_western.append(lyric_count)
                total_duration_western.append(duration)
            else:  # cjk
                total_cjk.append(lyric_count)
                total_duration_cjk.append(duration)
    
    # UPDATE: 根据中位数丢掉异常值 ——CYY
    def _ignore_anomaly(cnt_list, dur_list):
        x = np.log(np.array([dur / cnt for (cnt, dur) in zip(cnt_list, dur_list)]))
        median = np.median(x)
        mad = np.median(np.abs(x - median))
        z = 0.6745 * (x - median) / mad
        filtered_idx = [i for i in range(len(z)) if np.abs(z)[i] <= 3.5]
        filtered_cnt_list = [cnt_list[i] for i in filtered_idx]
        filtered_dur_list = [dur_list[i] for i in filtered_idx]
        return filtered_cnt_list, filtered_dur_list

    total_cjk, total_duration_cjk = _ignore_anomaly(total_cjk, total_duration_cjk)
    total_cjk, total_duration_cjk = sum(total_cjk), sum(total_duration_cjk)
    total_western, total_duration_western = _ignore_anomaly(total_western, total_duration_western)
    total_western, total_duration_western = sum(total_western), sum(total_duration_western)
...
def process_single_segment_list(segments):
    ...
    return processed_segments, duration_stats, has_reset
...
def process_json_data(data: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any], bool]:
    ...
    processed_segments, duration_stats, has_reset = process_single_segment_list(segments)
