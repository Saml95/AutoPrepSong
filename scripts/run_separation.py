...
def run_scp(model, args, config, device, verbose: bool = False):
    """
    Parameters:
    ----------
    ...
    args : Namespace
        Arguments containing input scp_file, output folder, and processing options.
    config : Dict
...
def proc_scp_file(dict_args):
    ...
    run_scp(model, args, config, device, verbose=True)
