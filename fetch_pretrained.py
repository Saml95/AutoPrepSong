...
def download_all(use_mirror: bool = False, local_path = "./ckpts"):
    ...
    urls = [
        (
            ...
            os.path.join(local_path, "MusicFM", "msd_stats.json"),
        ),
        (
            f"{base_url}/minzwon/MusicFM/resolve/main/pretrained_msd.pt",
            os.path.join(local_path, "MusicFM", "pretrained_msd.pt"),
        ),
        (
            f"{base_url}/ASLP-lab/SongFormer/resolve/main/SongFormer.safetensors",
            os.path.join(local_path, "SongFormer.safetensors"),
        ),
    ...
    snapshot_download("OpenMuQ/MuQ-large-msd-iter", local_dir=os.path.join(local_path, "MuQ-large-msd-iter"))



if __name__ == "__main__":
    local_path = sys.argv[1] if len(sys.argv) > 1 else "./ckpts"
    # By default, use HuggingFace. If you are in Mainland China, change to True
    download_all(use_mirror=False, local_path=local_path)
