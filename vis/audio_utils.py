...
class AudioNormalizer:
    """
    Audio normalization class that can be used as a callable.
    
    This class encapsulates audio normalization functions to adjust 
    audio volume to a target dB FS level and avoid clipping.
    ...
    def tailor_dB_FS(self, y):
        ...
        y_out = y * scalar
        return y_out, rms, scalar
    
    def avoid_clipping(self, y, scalar=None):
        """
        ...
        """
        if scalar is None:
            scalar = np.max(np.abs(y)) + self.eps
            if scalar < 1:
                scalar = 1 + self.eps
